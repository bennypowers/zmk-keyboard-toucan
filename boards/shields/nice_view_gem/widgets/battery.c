...
static void draw_hp_counter(lv_obj_t *canvas, int x, int y,
    ...
    /* Label: vertically centered with digit box */
    int label_y = y + (EB_DIGIT_H - EB_LABEL_H) / 2;
    lv_canvas_draw_img(canvas, x, label_y, label, &img_dsc);

    int dx = x + EB_LABEL_W + 2;
...
void draw_battery_status(lv_obj_t *canvas, const struct status_state *state) {
    draw_hp_counter(canvas, L_X, BATTERY_Y, &roll_l, &eb_hp_label);
    draw_hp_counter(canvas, R_X, BATTERY_Y, &roll_r, &eb_pp_label);
}
